 ...
 * Rules Engine
 *
 * @package GamiPress\BBForms\Rules_Engine
 * @since 1.0.0
 */
 ...
 * @return bool
 */
function gamipress_bbforms_check_if_meets_requirements( $requirement_id, $trigger, $args ) {

    // Initialize the return value
    $return = true;

    // If is specific category trigger
    if( $trigger === 'gamipress_bbforms_form_specific_category_submission' ) {

        $category_id = $args[2];
        $required_category_id = get_post_meta( $requirement_id, '_gamipress_bbforms_category', true );
    ...
    if( $trigger === 'gamipress_bbforms_form_specific_tag_submission' ) {

        $tag_id = $args[2];
        $required_tag_id = get_post_meta( $requirement_id, '_gamipress_bbforms_tag', true );

        // True if there is a specific tag, an attached form and both are equal
        $return = (bool) ( $tag_id === $required_tag_id );
    }

    if( $return && ( $trigger === 'gamipress_bbforms_field_value_submission'
            || $trigger === 'gamipress_bbforms_specific_field_value_submission' ) ) {

        $field_name = $args[2];
        $field_value = $args[3];

        $required_field_name = gamipress_get_post_meta( $requirement_id, '_gamipress_bbforms_field_name', true );
        $required_field_value = gamipress_get_post_meta( $requirement_id, '_gamipress_bbforms_field_value', true );
...
add_filter( 'user_has_access_to_achievement', 'gamipress_bbforms_user_has_access_to_achievement', 10, 6 );
 ...
 * @return array
 */
function gamipress_bbforms_filter_triggered_requirements( $triggered_requirements, $user_id, $trigger, $site_id, $args ) {
    ...
    foreach( $triggered_requirements as $i => $requirement ) {
        ...
        // Skip item
        if( ! gamipress_bbforms_check_if_meets_requirements( $requirement->ID, $trigger, $args ) ) {
            continue;
        }
...
add_filter( 'gamipress_get_triggered_requirements', 'gamipress_bbforms_filter_triggered_requirements', 20, 5 );
 ...
 */
function gamipress_bbforms_user_has_access_to_achievement( $return = false, $user_id = 0, $requirement_id = 0, $trigger = '', $site_id = 0, $args = array() ) {
    ...
    // Send back our eligibility
    return gamipress_bbforms_check_if_meets_requirements( $requirement_id, $trigger, $args );
